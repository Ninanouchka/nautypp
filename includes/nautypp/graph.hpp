...
public:
    ...
    Graph(const graph* G, size_t V, bool directed=false);
    ...
    /// \param directed Whether the graph is directed (default: false)
    Graph(graph* G, size_t V, bool copy=true, bool directed=false);

    /// \brief Constructor of empty graph
    ///
    /// \param V Number of vertices
    /// \param directed Whether the graph is directed (default: false)
    Graph(size_t V, bool directed=false);
    ...
    inline Graph copy() const {
        return Graph(g, n, true, _directed);
    }
    ...
    /// \brief Number of edges (or arcs) of the graph
    /// \return The (non-negative) number of edges/arcs of the graph
    ...
    /// \brief Get an iterator over all the arcs of a directed graph.
    ///
    /// Alias of edges() for directed graphs.
    /// \return An iterable over all arcs of the graph.
    inline AllEdges arcs() const {
        return edges();
    }

    /// \brief Get an iterator over all the out-arcs of a vertex in a directed graph.
    ///
    /// Alias of edges(v) for directed graphs.
    /// \param v The source vertex
    /// \return An iterable over all arcs leaving v.
    inline Edges out_arcs(Vertex v) const {
        return edges(v);
    }
    ...
    /// \return The degree (out-degree for directed) of v
    ///
    /// **Example**:
    ...
    /// \brief Get the out-degree of a vertex in a directed graph.
    ///
    /// Alias of degree() for directed graphs.
    /// \param v The vertex whose out-degree is returned
    /// \return The out-degree of v
    inline size_t out_degree(Vertex v) const {
        return degree(v);
    }

    /// \brief Get the in-degree of a vertex in a directed graph.
    ///
    /// Counts the number of vertices `u` such that an arc `u → v` exists.
    /// \param v The vertex whose in-degree is returned
    /// \return The in-degree of v
    inline size_t in_degree(Vertex v) const {
        size_t ret{0};
        for(Vertex u{0}; u < V(); ++u)
            if(ISELEMENT(GRAPHROW(g, u, _m), v))
                ++ret;
        return ret;
    }
    ...
    ///
    /// For directed graphs, returns out-degrees.
    ...
    /// \brief Add an edge (or arc) between two vertices.
    ///
    /// For undirected graphs, this adds an undirected edge between `v` and `w`.
    /// For directed graphs, this adds an arc `v → w` only.
    /// \param v,w Vertices to join
    ...
    /// \brief Add a directed arc `v → w`.
    ///
    /// Equivalent to link() on a directed graph.
    /// \param v Source vertex
    /// \param w Target vertex
    inline void add_arc(Vertex v, Vertex w) {
        return link(v, w);
    }

    /// \brief Remove an edge (or arc) between two vertices.
    ///
    /// For undirected graphs, removes the undirected edge between `v` and `w`.
    /// For directed graphs, removes only the arc `v → w`.
    /// \param v,w Vertices of the edge/arc to remove
    inline void remove_edge(Vertex v, Vertex w) {
        DELELEMENT(GRAPHROW(g, v, _m), w);
        if(!_directed)
            DELELEMENT(GRAPHROW(g, w, _m), v);
        degrees[v].set_stale();
        degrees[w].set_stale();
        nb_edges.set_stale();
        _as_cliquer.set_stale();
    ...
    /// Alias of remove_edge()
    inline void remove_arc(Vertex v, Vertex w) {
        return remove_edge(v, w);
    }

    /// \brief Get all the neighbours of a vertex
    ///
    /// For directed graphs, returns out-neighbours (successors).
    ...
    /// \brief Get the out-neighbours (successors) of a vertex in a directed graph.
    ///
    /// Alias of neighbours_of() for directed graphs.
    /// \param v The vertex whose out-neighbours are demanded
    inline Neighbours out_neighbours_of(Vertex v) const {
        return neighbours_of(v);
    }

    /// \brief Get the in-neighbours (predecessors) of a vertex in a directed graph.
    ///
    /// Returns vertices `u` such that an arc `u → v` exists.
    /// \param v The vertex whose in-neighbours are demanded
    inline InNeighbours in_neighbours_of(Vertex v) const {
        return {*this, v};
    }

    /// \brief Get the strongly connected components of a directed graph.
    ///
    /// See StronglyConnectedComponents.
    inline StronglyConnectedComponents get_strongly_connected_components() const {
    ...
    /// Remove all edges (or arcs) incident to some vertex.
    ///
    /// For directed graphs, removes all arcs leaving v and all arcs entering v.
    ...
    inline void isolate_vertex(Vertex v) {
        if(!_directed) {
            ...
            // Remove out-arcs: clear row v
            for(Vertex u{0}; u < n; ++u) {
                if(ISELEMENT(setwordof(v), u)) {
                    DELELEMENT(setwordof(v), u);
                    degrees[u].set_stale();
                }
            }
            // Remove in-arcs: clear bit v from every other row
            for(Vertex u{0}; u < n; ++u) {
                if(ISELEMENT(setwordof(u), v)) {
                    DELELEMENT(setwordof(u), v);
                    degrees[u].set_stale();
                }
            }
        }
    ...
    inline bool is_planar() const {
        if(_directed)
            throw std::runtime_error("is_planar() is not defined for directed graphs");
        const auto n{this->V()};
    ...
    friend class InNeighbourIterator;
...
/*      *************** Creation of Graph From Other Representations ***************      */
    ...
    /// \brief Construct a Graph from a row-flattened adjacency matrix.
    ///
    /// \param A The flattened adjacency matrix (V×V).
    /// \param upper true if \a A represents an upper triangular matrix and false otherwise.
    ///              Ignored when \a directed is true.
    /// \param directed true if the graph is directed (reads the full matrix).
    template <typename T>
    inline static Graph from_adjacency_matrix(
            const std::vector<T> A, bool upper=true, bool directed=false) {
        auto V{static_cast<size_t>(
            std::sqrt(static_cast<float>(A.size())) + .5
        )};
        return from_adjacency_matrix(A, V, upper, directed);
    }
    ...
    inline static Graph from_adjacency_matrix(
            ...
            size_t V, bool upper=true, bool directed=false) {
        if(directed)
            return from_full_adjacency_matrix(A, V);
        else if(upper)
    ...
    /// \brief Construct a directed Graph from a full row-major adjacency matrix.
    ///
    /// Each cell A[v*V + w] = 1 means an arc v → w exists.
    /// \param A The flattened adjacency matrix (V×V).
    /// \param V The number of vertices.
    template <typename T>
    static Graph from_full_adjacency_matrix(const std::vector<T> A, size_t V) {
        if(V*V != A.size())
            throw std::runtime_error("Wrong dimensions for adjacency matrix");
        Graph ret(V, true);
        for(Vertex v{0}; v < V; ++v)
            for(Vertex w{0}; w < V; ++w)
                if(v != w and A.at(v*V + w))
                    ret.add_arc(v, w);
        return ret;
    }
...
/*      *************** Construction of Graph Families ***************      */
    ...
    ///
    /// \param N The number of vertices.
    /// \return An empty directed graph on N vertices.
    inline static Graph make_directed(size_t N) {
        return Graph(N, true);
    }
